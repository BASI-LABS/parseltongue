...
import { toBase64, toLeetspeak } from "../utils";
import { AppState, DropdownSetting, GroupSetting, Setting, ToggleSetting, initialState, tiktokenModels } from "../types";


function isTiktokenModel(value: any): value is TiktokenModel {
    const models: TiktokenModel[] = [
      "davinci-002", "babbage-002", "text-davinci-003", "text-davinci-002", "text-davinci-001", "text-curie-001",
      "text-babbage-001", "text-ada-001", "davinci", "curie", "babbage", "ada", "code-davinci-002", "code-davinci-001",
      "code-cushman-002", "code-cushman-001", "davinci-codex", "cushman-codex", "text-davinci-edit-001",
      "code-davinci-edit-001", "text-embedding-ada-002", "text-similarity-davinci-001", "text-similarity-curie-001",
      "text-similarity-babbage-001", "text-similarity-ada-001", "text-search-davinci-doc-001", "text-search-curie-doc-001",
      "text-search-babbage-doc-001", "text-search-ada-doc-001", "code-search-babbage-code-001", "code-search-ada-code-001",
      "gpt2", "gpt-3.5-turbo", "gpt-35-turbo", "gpt-3.5-turbo-0301", "gpt-3.5-turbo-0613", "gpt-3.5-turbo-1106",
      "gpt-3.5-turbo-0125", "gpt-3.5-turbo-16k", "gpt-3.5-turbo-16k-0613", "gpt-3.5-turbo-instruct",
      "gpt-3.5-turbo-instruct-0914", "gpt-4", "gpt-4-0314", "gpt-4-0613", "gpt-4-32k", "gpt-4-32k-0314", "gpt-4-32k-0613",
...
const TextAreaWithCopyButton = ({ id, value, placeholder } : { id : string, value : string, placeholder : string}) => {
    ...
    return (
      <div className="flex-1 relative mb-4">
        <textarea
          id={id}
          className="w-full h-full p-2 pr-24 border border-gray-300 rounded-md bg-gray-50 text-gray-600 resize-none"
          disabled
          value={value}
        ...
        <button
          onClick={handleCopy}
          className="absolute right-2 top-2 p-[2px] bg-gray-300 text-white rounded-md hover:bg-gray-200 transition-colors duration-200"
        >
        { !copied ? 
  ...
  export default function SidePanel() {
      const [tokenizedText, setTokenizedText] = useState<JSX.Element[] | null>(null);
      const [leetspeakText, setLeetspeakText] = useState('');
      const [base64Text, setBase64Text] = useState('');
      const [encoder, setEncoder] = useState(getEncoding(getEncodingNameForModel('gpt-4o')));
                    ...
                    Object.entries((setting as GroupSetting).children).forEach(([childKey, childSetting]) => {
                      ...
                      if (childSetting.type === 'toggle' && fullKey in storedState) {
                        ...
                        if (
                            ...
                            isDropdownSetting(newState.settings.tokens.children.tokenizer)
                          ) {
                            const encoderName = newState.settings.tokens.children.tokenizer.value;
                            if (isTiktokenModel(encoderName)) {
                                try {
                                const newEncoder = getEncoding(getEncodingNameForModel(encoderName));
                                setEncoder(newEncoder);
                                } catch (error) {
                                console.error(`Failed to set encoder for ${encoderName}:`, error);
                                }
                            } else {
      ...
      const promptToTokens = (text: string, encoder: Tiktoken) => {
        ...
        return encoding.map((token: number, index: number) => {
            ...
            return <span className={colorClass} key={index}>{encoder.decode([token])}</span>;
        });
      }
      ...
      if (!state.isActive) {
          return <div className="flex items-center justify-center w-screen h-screen">
                  <div className="text-center">
                      ...
                      <p className='mt-4 text-md'>Currently Inactive, you need click the activate button in the pop</p>
                  </div>
                 </div>;
      }
  
        const showTokens: boolean = isGroupSetting(state.settings.tokens) &&
                                  isToggleSetting(state.settings.tokens.children.show_tokens) &&
                                                  state.settings.tokens.children.show_tokens.value;

        const showLeetspeak: boolean = isToggleSetting(state.settings.leetspeak) && state.settings.leetspeak.value;
        const showBase64: boolean = isToggleSetting(state.settings.base64) && state.settings.base64.value;
      ...
      <div className="h-full w-full max-w-md p-6 bg-white rounded-lg shadow-md flex flex-col">
        <h1 className="text-2xl font-bold mb-4 flex items-center">Utility Panel alpha ✨</h1>
        { showTokens && <div id="tokenizer"
             className="flex-1 p-2 border border-gray-300 rounded-md bg-gray-50 text-gray-600 resize-none mb-4 overflow-auto">
              {tokenizedText}
        </div>}
        { showLeetspeak && <TextAreaWithCopyButton id="leetspeak" value={leetspeakText} placeholder="Leetspeak output will appear here" />}
        { showBase64 && <TextAreaWithCopyButton id="base64" value={base64Text} placeholder="Base64 output will appear here" />}
      </div>
    </div>;
  }